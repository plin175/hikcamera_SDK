        ...
        if thread and thread.is_alive():
            ...
            if sys.platform == 'win32':
                try:
                    ...
                    handle = kernel32.OpenThread(THREAD_TERMINATE, False, thread_id)
                    if handle == 0:
                    ...
                    if kernel32.TerminateThread(handle, 0):
                        ...
                        for _ in range(10):  # 最多等待500ms
                            ...
                            self.logger.error("线程状态同步超时")
                    else:
                        error_code = get_last_error()
                        self.logger.error(f"终止失败 [0x{error_code:X}]")
            ...
            # 标记线程对象为已终止
            try:
                if hasattr(thread, '_is_stopped'):
                    thread._is_stopped = True  # 强制修改Python内部状态
                if hasattr(thread, '_tstate_lock'):
                    thread._tstate_lock = None
            except Exception as e:
                self.logger.error(f"修改线程状态时出现异常: {str(e)}")
